        ...
        while i < self.iters:  # test for very high number of runs
            ...
            if hash_val in self.name_dict.keys():
                ...
                if id != self.name_dict[hash_val]:
                    self.logger.debug("*******Collision occurred for {} bit hash {}, id {} and existing entry {}".format(
                        self.bits_hash, hash_val, id, self.name_dict[hash_val]))
                    i += 1
                    num_collisions += 1
            else:
                # this hash value was seen the first time. Save the entry
                # save this entry here for this hash value as this is the first time it is seen
                self.name_dict[hash_val] = id
                i += 1
...
def parseCmdLineArgs():
    ...
    parser.add_argument("-b", "--bits_hash", type=int, choices=[8, 16, 24, 32, 40, 48, 56, 64], default=32,
                        help="Number of bits of hash value to test for collision: allowable values between 6 and 64 in increments of 8 bytes, default 32")

    parser.add_argument("-e", "--num_entities", type=int, default=100,
                        help="Number of entities like pub, sub, discovery etc, default 100")

    parser.add_argument("-i", "--iters", type=int, default=1000000,
                        help="Number of iterations, default 1 million")

    parser.add_argument("-n", "--num_hosts", type=int, default=10,
                        help="Number of hosts in our mininet environment, default 10")

    parser.add_argument("-p", "--lower_port", type=int, default=5555,
                        help="lower bound of the port for whoever runs on the host, default 5555")

    parser.add_argument("-P", "--upper_port", type=int, default=7777,
                        help="upper bound of the port for whoever runs on the host, default 7777")
