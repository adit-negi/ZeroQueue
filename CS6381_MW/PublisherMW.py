...
class PublisherMW ():
    ...
    def handle_reply(self):
        ...
        try:
            ...
            elif disc_resp.msg_type == discovery_pb2.TYPE_ISREADY: #pylint: disable=no-member
    ...
    def register(self, name, topiclist):
        ...
        try:
            ...
            self.logger.debug(
                "PublisherMW::register - populate the Registrant Info")
            ...
            self.logger.debug(
                "PublisherMW::register - done populating the Registrant Info")

            # Next build a RegisterReq message
            self.logger.debug(
                "PublisherMW::register - populate the nested register req")
            ...
            self.logger.debug(
                "PublisherMW::register - done populating nested RegisterReq")

            # Finally, build the outer layer DiscoveryReq Message
            self.logger.debug(
                "PublisherMW::register - build the outer DiscoveryReq message")
            ...
            self.logger.debug(
                "PublisherMW::register - send stringified buffer to Discovery service")
            # we use the "send" method of ZMQ that sends the bytes
            self.req.send(buf2send)

            # now go to our event loop to receive a response to this request
            self.logger.info(
                "PublisherMW::register - sent register message and now now wait for reply")
    ...
    def is_ready(self):
        ...
        try:
            ...
            # now send this to our discovery service
            self.logger.debug(
                "PublisherMW::is_ready - send stringified buffer to Discovery service")
            # we use the "send" method of ZMQ that sends the bytes
            self.req.send(buf2send)

            # now go to our event loop to receive a response to this request
            self.logger.info(
                "PublisherMW::is_ready - request sent and now wait for reply")
    ...
    def disseminate(self, id, topic, data):
        ...
        try:
            ...
            self.logger.info("PublisherMW::disseminate - %s", send_str)
